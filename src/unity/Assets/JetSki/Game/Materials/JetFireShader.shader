Shader "Custom/JetFire" {
	
	Properties
	{
		_MainTex("Mask", 2D) = "white" {}
		_Color("Particle Color", Color) = (1, 1, 1)
...
#include "UnityCG.cginc"
	...
	sampler2D  _NoiseTex, _MainTex, _DistortTex;
	float4 _DistortTex_ST, _MainTex_ST, _NoiseTex_ST;
	...
	v2f vert(appdata v)
	{
		...
		o.uv2 = TRANSFORM_TEX(v.uv, _MainTex); // for the textures
	...
	half4 frag(v2f i) : COLOR
	{
		...
		fixed4 shapetex = tex2D(_MainTex, i.uv2); // mask texture
		...
		//finalcolor.a = Luminance(shapetex);
