        ...
        if (_playerControlled)
        {
            ...
            if (WaterBoosting)
            {
                ...
                if (forward != 0)
                {
                    ...
                    _rb.AddRelativeForce(0, 0, WaterThrust * forward, ForceMode.Acceleration);
                    _rb.AddRelativeTorque(Vector3.right * WaterThrust * (forward * 0.25f), ForceMode.Acceleration);
                    //_rb.AddRelativeForce(forwardVec3 * forward);
                    //_rb.AddRelativeForce(new Vector3(1, 0, 1).normalized * forward);
                }
                else
                {
                    _rb.AddRelativeForce(0, WaterThrust, 0, ForceMode.Acceleration);
                ...
                if (sideways != 0)
                {
                    _rb.AddRelativeTorque(Vector3.up * WaterThrust * (sideways * 0.25f), ForceMode.Acceleration);
                    //_rb.AddRelativeTorque(Vector3.forward * WaterThrust * (sideways * 0.25f), ForceMode.Acceleration);
                }
            }
            //if (CameraLockedOnBall) Camera.main.
        }
